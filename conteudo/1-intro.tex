...
As operações de adição e remoção de arestas são chamadas de \defi{modificações} do grafo dinâmico.

Um problema em grafos dinâmicos envolve verificar se para um dado $0\leq t\leq T$, $G_t$ satisfaz alguma determinada propriedade.
A operação de verificação dessa propriedade é chamada de \defi{consulta}.
Solucionar um tal problema infere desenvolver uma estrutura de dados capaz de dar suporte às modificações e consulta de forma eficiente.
