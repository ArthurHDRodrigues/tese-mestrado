...
A proposta desse trabalho de mestrado é estudar, implementar, comparar e apresentar,
de maneira didática, algoritmos para problemas relacionados a conexidade em grafos dinâmicos.
Concretamente, temos a intenção de estudar os dois seguintes problemas.

Um \defi{grafo dinâmico} é um par $(G, O)$ onde~$G$ é um grafo inicialmente dado e $O = (o_1, o_2, \ldots, o_T)$ é uma sequência de operações. Cada operação é ou a adição ou a remoção de uma aresta. Dessa forma, a primeira operação, $o_1$, é aplicada a $G$ e cada operação seguinte~$o_t$ é aplicada no grafo obtido pela aplicação das operações anteriores a ela.
...
\TODO{definir melhor o que é grafo dinâmico}
