...
A implementação rotina~\treapSplit($u$) pode ser vista no Algoritmo~\ref{Algo:TREAPsplit}. Nesse algoritmo mantemos três ponteiros: $L$, $R$ e~$tmp$. 
\begin{algorithm}
\caption{\treapSplit($uu$)}
\label{Algo:TREAPsplit}
\begin{algorithmic}[1]
\If { $uu$ $=$ \Nil} \Return \Nil, \Nil
\EndIf
\State $R$ $\gets $ $u$.$dir$
\State $L$ $\gets $ $u$.$esq$
\State $tmp$ $\gets$ $uu$
