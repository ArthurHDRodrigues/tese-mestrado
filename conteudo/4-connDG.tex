...
\todo{Para tirar a impressão de que \hashCreate{} gasta espaço quadrático, removi o $n^2$ que estava como parâmetro.}
...
Para concluir essa seção, argumentaremos como ocorre a amortização de custo de \dymGraphDelEdge{}. Notemos que é difícil calcular o número exato de arestas percorridas em uma única execução de \dymGraphReplace{}. No entanto, podemos calcular tal número ao longo de uma sequência de remoções de arestas. \NEW{Suponhamos que um grafo dinâmico inicialmente vazio sofreu $m$ inserções de arestas e que agora iremos remover cada uma delas. No pior dos casos, cada aresta é rebaixada $\lceil \lg n \rceil$ vezes, percorrendo assim todos os níveis da nossa estrutura de dados, e como cada rebaixamento custa $\O{\lg n}$, teremos que o custo total de todos os percussos em todas as remoções de arestas é~$\O{m\lg^2 n}$. Logo o custo amortizado de cada remoção é $\O{\lg^2 n}$.}
