...
\NEW{
Denotando por $|T|$ o número de vértices de uma árvore~$T$.
}
...
Provaremos esse fato por contradição, supondo que exista uma aresta~$xy$ substituta a~$uv$ com nível $j<i$ e, sem perda de generalidade, supondo que~$x$ é um vértice de~$T_u$ e~$y$ é de~$T_v$.
\NEW{
Como $xy$ é uma aresta de nível~$j$, temos que~$xy$ é uma aresta de~$G_{\leqslant j}$ e logo que~$x$ e~$y$ estão na mesma componente de $G_{\leqslant j}$, e portanto, pela invariante~\ref{invar:SF}, estão na mesma componente conexa de~$F_{\leqslant j}$.
Vamos analisar o caminho~$P$ que liga~$x$ a~$y$.
...
Como $x$ e $y$ estão em componentes distintas quando removemos $uv$, temos que $uv$ é uma aresta de~$P$ e assim que $uv$ está em $F_{\leqslant j}$, o que contrária o fato de $uv$ ser uma aresta de nível~$i$.
}
...
\NEW{
que ocorre somente no laço da linha~\ref{Algo:dymGraphReplace:linha:moveTu} do Algoritmo~\ref{Algo:dymGraphReplace}.
Nessa ocasião, precisamos percorrer o caminho do nó que representa a aresta rebaixada até sua raiz decrementando~$cniv$, o que consome tempo esperado~$\O{\lg n}$. Como cada iteração desse laço já consome tempo esperado~$\O{\lg n}$, manter esse campo não prejucida o consumo de tempo da rotina.

Além disso, precisamos manter~$cniv$ e~$cres$ atualizado nas operações~\treapSplit{} e~\treapJoin{} da biblioteca de treaps, pois essas rotinas modificam a estrutura das treaps. A atualização desses campos consome tempo constante para cada nó da treap afetado pela modificação. Assim não onerando o consumo de tempo dessas rotinas. 


Além dessas ocasiões, o contador~$cres$, exige atualizações em situações adicionais.
}
