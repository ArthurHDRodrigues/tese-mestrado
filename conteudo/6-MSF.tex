...
Formalmente, uma \defi{imersão no plano} de um grafo~$G=(V,E)$ é um par de funções~$\phi_V:V\mapsto \R^2$ e $\phi_E: E\times [0,1]\mapsto \R^2$ tais que:
\begin{enumerate}
...
Uma dessas faces é ilimitada, essa face é chamada de \defi{face exterior}.
...
A aresta~$e^\star$ é chamada de \defi{aresta dual} de~$e$ e os vértices incidentes a essa aresta são as duas faces cujas fronteiras contém~$e$.
Nesse contexto, chamamos~$G$ de grafo \defi{primal}.
Para ilustrar a relação entre cada aresta e seu dual, geralmente desenhamos cada aresta do grafo dual cruzando as aresta do grafo primal, como mostra a Figura \TODO{2}.
...
Dado um grafo plano~$G$ e uma descrição combinatória~$D$ da imersão de~$G$, um \defi{ciclo facial} de~$D$ é um ciclo~$v_0,v_1,\ldots,v_{k-1},v_0$ onde $v_iv_{i+1}$ é sucessor de $v_iv_{i-1}$ em $D(v_i)$. A Figura \TODO{bla} mostra ciclos faciais do grafo da Figura \TODO{bla}.
