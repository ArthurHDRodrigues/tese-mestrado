...
Cada componente~$C$ de~$G$ é representada por duas árvores com pesos em seus vértices, sendo a primeira construida a partir de uma árvore geradora de peso mínimo~$T$ de~$C$ e a segunda a partir da correspondente dual~$T^\star$.
...
Os quatro primeiros, denotados por~$e_0$, $e_1$, $e_2$ e~$e_3$, representa $e$.
...
Os quatro últimos, denotados por~$v_0$, $v_1$, $v_2$ e~$v_3$, mantém a posição de~$e$ e~$e^\star$ nas ordens cíclicas.
O nó~$v_i$ é ligado ao nó~$e_i$.
...
Manteremos essas duas ADPs e uma tabela de símbolos $H$, que guarda o conjunto de octuplas de nós $e_0$, $e_1$, $e_2$, $e_3$, $v_0$, $v_1$, $v_2$ e~$v_3$ e usa como chaves as arestas de~$G$.
...
Cada nó em uma link-cut tree mantém informações sobre seus pais e filhos, além de possíveis conexões reversas devido à natureza dinâmica da estrutura.
A estrutura interna das link-cut trees é composta por árvores de busca binária, especificamente utilizando uma variante conhecida como Splay Tree.
Essa organização permite que operações como acesso, corte, e união de árvores sejam realizadas de forma eficiente, aproveitando as propriedades de balanceamento e acesso rápido das Splay Trees.

Link-cut trees são especialmente úteis em algoritmos que necessitam de operações dinâmicas sobre árvores, como em problemas de análise de conectividade em grafos dinâmicos.
...
$u$ e $v$ vértices
$p$ nó na ordem cíclica $D(u)$.
$q$ nó de link cut tree de $D(v)$
...
\item \LCOAddCost($v$, $w$): Atribui o peso~$w$ ao vértice~$v$.
...
\State \LCOCycle($e_0$)
\State \LCOSplit(\LCOPredecessor($e_0$))
\State \LCOCycle($e_2$)
\State \LCOSplit(\LCOPredecessor($e_2$))
\State \LCOCycle($e_1$)
\State \LCOSplit(\LCOPredecessor($e_1$))
\State \LCOCycle($e_3$)
\State \LCOSplit(\LCOPredecessor($e_3$))
